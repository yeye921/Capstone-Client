...
export default function Here({ state }) {
	...
	useEffect(() => {
		axios
			.all([
				...
				axios.get(`http://3.39.164.26/chat/nanumPlaceInfo?pId=${Info.state.pId}`)
			])
			.then(
	...
	const getData = async () => {
		await axios.get(`http://3.39.164.26/chat?pId=${Info.state.pId}`).then((data) => {
			let users = data.data;
			if (users.length !== 0) {
				setUserInfo(users);
			}
	...
	const getPLocation = async () => {
		await axios.get(`http://3.39.164.26/chat/nanumPlaceInfo?pId=${Info.state.pId}`).then((response) => {
			setData({
				x: response.data.x,
				y: response.data.y
			});
